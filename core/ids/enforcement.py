...
class Enforcement:
    ...
    def apply(self, decision, flow):
        # Start building the behavior summary
        behavior_summary = self._analyze_behavior(flow)
        
        # Append camera/mic activity if detected
        cam_mic_behavior = []
        ...
        msg = (
            "⚠️ Activity Detected\n"
            f"Process: {flow['proc']}\n"
            f"Target: {flow['r_ip']}:{flow['r_port']}\n"
        ...
        # Enforce block if required by the decision model
    ...
    def _analyze_behavior(self, flow):
        ...
        try:
            ...
            # Use a generic "Running process" message
            ...
            try:
                ...
                pass # Can't access connections, just move on
